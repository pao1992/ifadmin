<template>
    <div class="spec-container">
        ...
        <dl style="display: flex;clear: both">
            <p>新增</p>
            <input class="el-button el-button--small" size="mini" placeholder="规格名称"
                   style="width: 100px;margin-left: 10px;" v-model="addRow.name"/>
            <input class="el-button el-button--small" size="mini" placeholder="选项"
                   style="width: 100px;margin-left: 10px;" v-model="addRow.options[0].item"/>
        ...
        <dl v-for="(row,index) in list">
            ...
            <input
                ...
                style="width: 100px;"
            />
            <el-button v-if="currentIndex !== index" class="button-new-tag" size="small" @click="currentIndex = index">
                +选项
            </el-button>
        </dl>
        ...
        <div>
            <table id="spec_input_tab" class='table'>
                ...
                <tr v-for="(g,index) in group">
                    <td v-for="i in g.key_name">{{i}}</td>
                    <td><input class="el-input__inner" v-model="group[index].price" @change="addToVuex" type="number"/>
                    </td>
                </tr>
                </tbody>
            </table>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        mounted(){
            //初始化
            this.list = this.product.spec.required_single.items;
            this.group = this.product.spec.required_single.items_price;
...
//            })
        ...
        computed: {
            ...
            addItem(){
                ...
                return {'item': '', 'name': this.list[this.currentIndex].name}
            }
        },
        watch: {
            list(){
        ...
        methods: {
            ...
            addToVuex() {
                ...
                data.items_price = this.group
                data.num_related = true
                data.required = 1
                this.UPDATE_SPEC({type: 'required_single', data: data})
            }
